package example.testtask.cities_roads.dijkstra.a;
...
/**
** {@link https://github.com/akoubensky/algo/blob/master/Lecture21/src/dijkstra/Dijkstra.java}
* ********************************************************************************************
* Реализация алгоритма Дейкстры поиска минимальных путей в графе с неотрицательной нагрузкой на дугах.
* Хранение текущих расстояний до вершин графа производится в двоичной куче.
*/
...
//        System.out.println("Shortest path from " + source + " to " + dest + ": " + dijkstra.getPath(source, dest));
        ...
        Graph g = new Graph(10);
        /* Начало дуги (номер вершины) || Конец дуги (номер вершины) || Нагрузка на дугу */
//        g.addArc(1,2,150);
//        g.addArc(2,3,100);
//        g.addArc(3,4,150);
//        g.addArc(4,5,150);
...
//        g.addArc(4,6, 200);
        ...
        Dijkstra dijkstra = new Dijkstra(g);
        System.out.println("Список вершин на пути от начальной (1) вершины к конечной (4): " + dijkstra.getPath(1, 4));
        System.out.println("Выдает дерево минимальных путей (Номер исходной вершины): " + Arrays.toString(dijkstra.getTree(1))); // (1) 4,-1,1,2,3,-1   (3) 4,0,1,-1,3,-1  (0) -1,0,1,2,3,-1
        System.out.println("Выдает длины минимальных путей (Номер исходной вершины): " + Arrays.toString(dijkstra.getDistances(1)));
    }
}
